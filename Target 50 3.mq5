//+------------------------------------------------------------------+
//| Gold $50 NONSTOP TREND + BREAKOUT CONTINUE + PROTECT v3 |
//+------------------------------------------------------------------+
#property strict
...
input int MaxTrades = 2; // allow 2 for breakout add
...
input int BreakoutPoints = 80; // breakout over resistance
...
int OnInit(){ h1_fast=iMA(_Symbol,PERIOD_H1,50,0,MODE_EMA,PRICE_CLOSE); h1_slow=iMA(_Symbol,PERIOD_H1,200,0,MODE_EMA,PRICE_CLOSE); m1_fast=iMA(_Symbol,PERIOD_M1,5,0,MODE_EMA,PRICE_CLOSE); m1_slow=iMA(_Symbol,PERIOD_M1,13,0,MODE_EMA,PRICE_CLOSE); m1_rsi=iRSI(_Symbol,PERIOD_M1,14,PRICE_CLOSE); trade.SetExpertMagicNumber(MagicNumber); EventSetTimer(60); CreateButton(); lastDay=iTime(_Symbol,PERIOD_D1,0); dayStartBalance=AccountInfoDouble(ACCOUNT_BALANCE); CalculateSR(); lastSRUpdate=TimeCurrent(); return(INIT_SUCCEEDED); }
void OnDeinit(const int r){ EventKillTimer(); ObjectDelete(0,"BTN_STARTSTOP"); ObjectDelete(0,"M15_RESISTANCE"); ObjectDelete(0,"M15_SUPPORT"); ObjectDelete(0,"SWEEP_LEVEL"); ObjectDelete(0,"PROGRESS_BG"); ObjectDelete(0,"PROGRESS_BAR"); IndicatorRelease(h1_fast); IndicatorRelease(h1_slow); IndicatorRelease(m1_fast); IndicatorRelease(m1_slow); IndicatorRelease(m1_rsi); }
void OnTimer(){ if(TimeCurrent()-lastSRUpdate>=SR_Update_Seconds){ CalculateSR(); lastSRUpdate=TimeCurrent(); } UpdateDrawings(); }
void OnTick()
{
   CheckNewDay();
   double floating=GetFloatingProfit(); double totalToday=dailyRealized+floating;
   if(totalToday>=DailyProfitTarget){ CloseAll("BLOCK HIT"); totalBlocksHit++; dailyRealized=0; dayStartBalance=AccountInfoDouble(ACCOUNT_BALANCE); lastDay=iTime(_Symbol,PERIOD_D1,0); return; }
   ...
   int bias=GetH1Bias(); if(bias==0){ UpdateDrawings(); ManageTrades(); return; }
   if(CountTrades()<MaxTrades)
   {
      if(currentState==WAIT_SWEEP){ CheckForSweep(bias); CheckForTrendPullback(bias); CheckForBreakoutContinuation(bias); }
...
void UpdateDrawings(){ DrawHLine("M15_RESISTANCE",ResLevel,clrRed); DrawHLine("M15_SUPPORT",SupLevel,clrLime); int bias=GetH1Bias(); double floating=GetFloatingProfit(); double totalToday=dailyRealized+floating; double pct=DailyProfitTarget>0? totalToday/DailyProfitTarget*100:0; if(pct<0) pct=0; if(pct>100) pct=100; DrawProgress(pct,totalToday); Comment(StringFormat(" NONSTOP TREND+BREAKOUT | H1:%s RES:%.2f SUP:%.2f BLOCK %d $%.2f/$%.2f", bias==1?"BUY":bias==-1?"SELL":"NONE",ResLevel,SupLevel,totalBlocksHit+1,totalToday,DailyProfitTarget)); }
void DrawHLine(string n,double p,color c){ if(p<=0) return; if(ObjectFind(0,n)<0) ObjectCreate(0,n,OBJ_HLINE,0,0,p); ObjectSetDouble(0,n,OBJPROP_PRICE,p); ObjectSetInteger(0,n,OBJPROP_COLOR,c); ObjectSetInteger(0,n,OBJPROP_WIDTH,2); }
void DrawProgress(double pct,double t){ string bg="PROGRESS_BG"; string bar="PROGRESS_BAR"; if(ObjectFind(0,bg)<0){ ObjectCreate(0,bg,OBJ_RECTANGLE_LABEL,0,0,0); ObjectSetInteger(0,bg,OBJPROP_XDISTANCE,20); ObjectSetInteger(0,bg,OBJPROP_YDISTANCE,65); ObjectSetInteger(0,bg,OBJPROP_XSIZE,200); ObjectSetInteger(0,bg,OBJPROP_YSIZE,20); ObjectSetInteger(0,bg,OBJPROP_BGCOLOR,clrDimGray); } if(ObjectFind(0,bar)<0){ ObjectCreate(0,bar,OBJ_RECTANGLE_LABEL,0,0,0); ObjectSetInteger(0,bar,OBJPROP_XDISTANCE,20); ObjectSetInteger(0,bar,OBJPROP_YDISTANCE,65); ObjectSetInteger(0,bar,OBJPROP_YSIZE,20); } int w=(int)(200*pct/100.0); ObjectSetInteger(0,bar,OBJPROP_XSIZE,w); color cc= pct>=100?clrLime: pct>=50?clrGold:clrDodgerBlue; ObjectSetInteger(0,bar,OBJPROP_BGCOLOR,cc); }

// BREAKOUT CONTINUATION - THIS CATCHES YOUR SCREENSHOT
void CheckForBreakoutContinuation(int bias)
{
   double f[1],s[1],r[1]; if(CopyBuffer(m1_fast,0,0,1,f)<=0) return; if(CopyBuffer(m1_slow,0,0,1,s)<=0) return; if(CopyBuffer(m1_rsi,0,0,1,r)<=0) return;
   double bid=SymbolInfoDouble(_Symbol,SYMBOL_BID); double ask=SymbolInfoDouble(_Symbol,SYMBOL_ASK);
   // BUY BREAKOUT: price above RES + BreakoutPoints, H1 BUY, M1 up
   if(bias==1 && bid > ResLevel + BreakoutPoints*_Point)
   {
      if(f[0]>s[0] && r[0]>55 && r[0]<80)
      {
         if(CountTrades()<MaxTrades)
         {
            PrintFormat("BREAKOUT BUY CONTINUE above RES %.2f -> %.2f",ResLevel,bid);
            OpenTrade(ORDER_TYPE_BUY);
            ResLevel = bid; // update resistance to new level
            DrawHLine("M15_RESISTANCE",ResLevel,clrRed);
         }
      }
   }
   if(bias==-1 && ask < SupLevel - BreakoutPoints*_Point)
   {
      if(f[0]<s[0] && r[0]<45 && r[0]>20)
      {
         if(CountTrades()<MaxTrades)
         {
            PrintFormat("BREAKOUT SELL CONTINUE below SUP %.2f -> %.2f",SupLevel,ask);
...
void ManageTrades()
{
   ...
   {
      if(PositionGetSymbol(i)!=_Symbol) continue; if((long)PositionGetInteger(POSITION_MAGIC)!=MagicNumber) continue;
      double open=PositionGetDouble(POSITION_PRICE_OPEN); double curr=PositionGetDouble(POSITION_PRICE_CURRENT); double sl=PositionGetDouble(POSITION_SL); long pType=PositionGetInteger(POSITION_TYPE); ulong ticket=(ulong)PositionGetInteger(POSITION_TICKET);
      double pts=(pType==POSITION_TYPE_BUY)? (curr-open)/_Point : (open-curr)/_Point; bool isBuy=(pType==POSITION_TYPE_BUY);
      ...
      {
         double f[1],s[1],r[1]; CopyBuffer(m1_fast,0,0,1,f); CopyBuffer(m1_slow,0,0,1,s); CopyBuffer(m1_rsi,0,0,1,r);
         // Reversal protect
         if(isBuy && f[0]<s[0] && r[0]<50){ if(trade.PositionClose(ticket)) continue; }
         if(!isBuy && f[0]>s[0] && r[0]>50){ if(trade.PositionClose(ticket)) continue; }
         if(isBuy && bias==-1){ if(trade.PositionClose(ticket)) continue; }
         if(!isBuy && bias==1){ if(trade.PositionClose(ticket)) continue; }
         // Trail 100 pts after +150
         if(pts>=150){ double newSL=isBuy? curr-100*_Point : curr+100*_Point; bool improve=isBuy? newSL>sl : newSL<sl; if(improve) trade.PositionModify(ticket,newSL,0); }
      }
      // MODIFIED: Don't close at RES if strong breakout - only close if weak
      double bid=SymbolInfoDouble(_Symbol,SYMBOL_BID);
      double ask=SymbolInfoDouble(_Symbol,SYMBOL_ASK);
      bool strongBreakoutBuy = isBuy && bid > ResLevel + BreakoutPoints*_Point && bias==1;
      bool strongBreakoutSell =!isBuy && ask < SupLevel - BreakoutPoints*_Point && bias==-1;
      if(strongBreakoutBuy || strongBreakoutSell)
      {
         // HOLD breakout - don't close at old resistance
         continue;
      }
      // Normal close at opposite level only if not breaking out
      if(!isBuy){ if( (curr <= SupLevel+50*_Point && pts>=100) || pts>=350 ) trade.PositionClose(ticket); }
      else { if( (curr >= ResLevel-50*_Point && pts>=100 &&!strongBreakoutBuy) || pts>=350 ) trade.PositionClose(ticket); }
   }
}
...
void CloseAll(string r){ for(int i=PositionsTotal()-1;i>=0;i--){ if(PositionGetSymbol(i)!=_Symbol) continue; if((long)PositionGetInteger(POSITION_MAGIC)!=MagicNumber) continue; ulong ticket=(ulong)PositionGetInteger(POSITION_TICKET); trade.PositionClose(ticket); } }
...
void OnChartEvent(const int id,const long &l,const double &d,const string &s){ if(id==CHARTEVENT_OBJECT_CLICK && s=="BTN_STARTSTOP"){ TradingEnabled=!TradingEnabled; UpdateButton(); ChartRedraw(); } }
